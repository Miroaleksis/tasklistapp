	...
	taskList.className = 'tasklist';
		...
		taskListHeader.setAttribute('aria-label', 'Task list');
			...
			name.setAttribute('maxlength', '60')
				...
				// Leave the textfield with enter
				name.addEventListener('keydown', (event) => {
					if (event.key === 'Enter') {
						name.blur(); // Poista fokus, mikä lopettaa muokkauksen
					}
				});
			...
			addTaskButton.textContent = 'Add new task';
				...
				addTaskButton.addEventListener('click', () => {
					...
					task.setAttribute('aria-label', 'Task');
						...
						taskDescription.contentEditable = true; // Teksti on muokattavissa
					...
					taskDescription.focus(); // Text focus
